...
router.get('/', function(req, res, next) {
  db.User.findAll({raw:true}).then(function(tod){
    db.Todo.findAll({include:db.User}).then(function(resus){
        res.render('pages/memo', { title:'Todo List',tododata:resus, users:tod});
    })
  })
});
...
router.post('/', function(req, res, next) {
    ...
    db.Todo.create({title: req.body.title,is_complete:false,completedAt:new Date(),UserId:req.body.UserId}).then(function(tod){
      res.redirect('/')
    })
